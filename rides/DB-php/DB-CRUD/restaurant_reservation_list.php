...
// 設定一頁最多幾筆資料
$perPage = 25;
$t_sql = "SELECT COUNT(1) FROM restaurant_reservation";
// 資料總筆數
...
if ($totalRows > 0) {
    ...
    $sql = sprintf(
    "SELECT * FROM restaurant_reservation ORDER BY restaurant_reservation_id DESC LIMIT %s, %s",
    ($page - 1) * $perPage,
    $perPage
    ...
    <table class="table table-bordered table-striped">
        ...
        <tr>
            <td>
                ...
                <a href="javascript: deleteItem(<?= $r['restaurant_reservation_id'] ?>)"><i class="fa-solid fa-trash-can"></i></a>
            </td>
            <td><?= $r['restaurant_reservation_id'] ?></td>
            <td><?= $r['shop_id'] ?></td>
            <td><?= $r['open_week'] ?></td>
            ...
            <td>
                <a href="edit.php?restaurant_reservation_id=<?= $r['restaurant_reservation_id'] ?>"><i class="fa-solid fa-pen-to-square"></i></a>
            </td>
        </tr>
        <?php endforeach ?>
        </tbody>
...
function deleteItem(restaurant_reservation_id) {
    if (confirm(`確定要刪除編號為 ${restaurant_reservation_id} 的資料嗎?`)) {
        location.href = 'delete.php?restaurant_reservation_id=' + restaurant_reservation_id;
    }
    }</script>
