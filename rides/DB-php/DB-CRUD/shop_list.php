...
if ($totalRows > 0) {
    ...
    $sql = sprintf(
    "SELECT * FROM shop ORDER BY shop_id DESC LIMIT %s, %s",
    ($page - 1) * $perPage,
    $perPage
    ...
    <table class="table table-bordered table-striped">
        ...
        <tr>
            <td>
                ...
                <a href="javascript: deleteItem(<?= $r['shop_id'] ?>)"><i class="fa-solid fa-trash-can"></i></a>
            </td>
            <td><?= $r['shop_id'] ?></td>
            <td><?= htmlentities($r['shop_name']) ?></td>
            <td><?= $r['shop_img'] ?></td>
            <td><?= $r['shop_longitude'] ?></td>
            <td><?= $r['shop_latitude'] ?></td>
            ...
            <td><?= htmlentities($r['shop_description']) ?></td>
            ...
            <td>
                <a href="edit.php?shop_id=<?= $r['shop_id'] ?>"><i class="fa-solid fa-pen-to-square"></i></a>
            </td>
        </tr>
        <?php endforeach ?>
        </tbody>
...
function deleteItem(shop_id) {
    if (confirm(`確定要刪除編號為 ${shop_id} 的資料嗎?`)) {
        location.href = 'delete.php?shop_id=' + shop_id;
    }
    }</script>
