...
$isPass = true;
  if(empty($_POST['ride_support_name']))
    {// {檢查不通過
      $isPass = false;
    // 輸出錯誤訊息error{'form':''}
      $output['errors']['ride_support_name'] = '請輸入支援種類名稱';
    }
    if(empty($_POST['ride_support_description']))
    {// {檢查不通過
      $isPass = false;
    // 輸出錯誤訊息error{'form':''}
      $output['errors']['ride_support_description'] = '請輸入支援種類簡述';
    }
...
$sql = "INSERT INTO `ride_support`(
  `ride_support_name`, `ride_support_description`
  ) VALUES (
    ?, ?
  )";
...
$stmt->execute([
  ...
  $_POST['ride_support_name'],
  $_POST['ride_support_description']
]);
